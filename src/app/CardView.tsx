...
function CardStyle(card: Cardlike): React.CSSProperties {
  return {
    fontWeight: (card.styling.emphasize && 'bold') || undefined,
    textDecoration:
...
const EdgeBinView = (props: { parent: Cardlike; bin: DisplayBin }) => (
  <div
    style={{
      minWidth: '10em',
    }}
  >
    <div>
      <b>{props.bin.relationship}</b>
    </div>
    <br />
    {props.bin.neighbors.map(card => (
      <div
        key={`${props.parent.id}-${card.id}`}
        style={{
          marginBottom: '0.3em',
        }}
      >
...
export const CardView = (props: {
  ...
  const edgeTo = edges
    .filter(e => e.related[0].id === card.id)
    .map(edge => ({
      relationship: edge.relationship,
      neighbor: edge.related[1],
    }));
  const edgeFrom = edges
    .filter(e => e.related[1].id === card.id)
    .map(edge => ({
      relationship:
        deck.data.relationshipInverse[edge.relationship] ?? edge.relationship,
      neighbor: edge.related[0],
    }));
  const edgeBins: DisplayBin[] = sortBy(
    groupBy([...edgeTo, ...edgeFrom], e => e.relationship).map(rel => ({
      relationship: rel[0].relationship,
      neighbors: sortBy(
        unique(rel.map(rel => rel.neighbor)),
        c => c.displayName,
      ),
    })),
    bin => bin.relationship,
  );
